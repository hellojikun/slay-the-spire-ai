...
Updated 2026-07-05 after the probe73 route/readiness validation round.
...
- Latest included probe: `ai_runs_strategy_probe73/20260705_160652_ironclad_a4.jsonl` ended as a clean F6 Sentries game-over after 215 action records, 1 recovered preflight mismatch, and 0 failed actions. It showed that F3 chose a no-buffer forced-elite path over a shop, then died in Sentries with no AOE/Weak and low HP. `ai_runs_parallel_flow/20260705_155559_ironclad_a4.jsonl` is the flow-agent confirmation of the same route/resource bottleneck: F7 had 37/88 HP and the route was already locked into `T -> E`. `ai_runs_strategy_probe72/20260705_153721_ironclad_a4.jsonl` remains the earlier F11 clean failure evidence.
...
- Latest learning run: `python -m slay_ai.training_manifest ...probe64 ...probe72 --output data\training_manifest_probe64_72.json --shadow-dir data\shadow_probe64_72 --knowledge-dir data\static_knowledge` classified 9 clean logs, 0 diagnostic logs, and 0 infra-blocked logs, then wrote static-knowledge and readiness-enriched route-risk, potion-tempo, and pre-boss deck-quality rows. The flow-agent manifest `data\training_manifest_parallel_flow.json` classified 1 clean log, and probe73 manifest `data\training_manifest_probe73.json` classified 1 clean log.
- Latest validation: `python -m unittest discover -s tests` ran 192 tests OK; `python -m compileall slay_ai tests` OK. Probe73 shows clean execution but a strategic route/combat failure, so the next live validation should check whether the stronger shop-buffer route rule avoids the F3 no-buffer forced elite.
- Next live validation: continue Stage 4. Active bottlenecks are early Act 1 forced-elite route commitment, Sentries AOE/Weak/readiness, high-pressure potion/card-sequence search, pre-boss deck/resource quality, Hexaghost/boss high-pressure survival, Slime Boss split/minion handling, and early Act 2 low-HP route/resource planning.
