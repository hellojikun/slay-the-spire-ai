...
Updated 2026-07-05 after the A0 Act 1 boss repeat probe, CHEST skipped-relic validation, and run-log directory migration.
...
- Current short-term milestone: stabilize A0 Act 1 boss completion before resuming ascension climbing. Probe91 (`runs/ai_runs_strategy_probe91_a0/20260705_201624_ironclad_a0.jsonl`) reached F19 after beating Slime Boss but was manually stopped. Probe94 (`runs/ai_runs_strategy_probe94_a0/20260705_204121_ironclad_a0.jsonl`) repeated the Act 1 clear, reached Act 2 F31, and is classified clean/trainable in `data/training_manifest_probe94_a0.json`. This is progress from "reach boss" to "repeat Act 1 clear", but not yet stability proof.
...
- Latest probe94 manifest: `data\training_manifest_probe94_a0.json` classified 1 clean log, 0 diagnostic logs, and 0 infra-blocked logs, with 29 route-risk rows, 193 potion-tempo rows, and 1 pre-boss deck-quality row. The run had 413 ok actions, 3 recoverable action races, 1 preflight mismatch, 0 failed actions, and a synthetic GameOver after MCP state tools returned `Internal error: null`.
...
- Next live validation: after the terminal-state MCP fix, run another A0 probe under `runs/ai_runs_strategy_probe95_a0` to verify repeated Act 1 clear, clean terminal handling, and lower action-race count. Active bottlenecks are MCP `GAME_OVER` null-state serialization, action/state freshness around reward/choice transitions, Act 2 low-HP route risk, potion sequencing, and runner target/settle races.
