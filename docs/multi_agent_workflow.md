...
Updated 2026-07-05 after the probe78 Slime Boss resource-quality round.
...
- Latest included probes: `ai_runs_strategy_probe80/20260705_174222_ironclad_a4.jsonl` ended as a clean F16 Guardian game-over after 312 ok actions, 1 recovered action, and 0 failed actions; Battle Trance was not in the deck, so the probe79 draw-order rule did not trigger, but the run exposed a Guardian search-commitment drift where accepted `Defend -> Whirlwind` lines replanned into weaker single-card attacks. `ai_runs_strategy_probe79/20260705_172700_ironclad_a4.jsonl` ended as a clean F16 Guardian game-over after 230 ok actions, 2 recovered actions, and 0 failed actions; it reached the boss with AOE but no potions, then died on a 36-incoming Guardian turn where draw/order and Mode Shift planning were insufficient. `ai_runs_strategy_probe78/20260705_171459_ironclad_a4.jsonl` ended as a clean F16 Slime Boss game-over after 267 ok actions, 3 recovered actions, and 0 failed actions.
...
| --- | --- | --- | --- |
| Archimedes | Flow / run-data agent | `019f30f8-ed4d-7bd3-9519-b3bcfda8116d` | Run frontier probes, accumulate climb logs, classify whether failures are clean/diagnostic/infra, and report concrete bottlenecks. Does not edit code. Current assignment: run probe81 after the Guardian attack-followup commitment fix and write only probe logs/manifests. |
| Harvey | AI upgrade / algorithm agent | `019f2e94-552b-7301-9cc5-21eed1fc2949` | Static features, readiness gates, shadow models, training data design, and model promotion gates. Does not edit code unless explicitly reassigned. Current assignment: evaluate Slime Boss/resource-quality and learning-model priorities. |
