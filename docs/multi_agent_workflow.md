...
Updated 2026-07-05 after the probe70 non-attack damage modeling validation.
...
- Latest included probe: `ai_runs_strategy_probe70/20260705_150111_ironclad_a4.jsonl` reached F16 Hexaghost and ended as a clean synthetic game-over. It validates that non-attack `damage: 1` noise is no longer destabilizing action execution, and it exposed the next major Stage 4 issue: one-turn search often finds a multi-card defensive line, but only the first action is committed and later replans drift back to attacks. `ai_runs_strategy_probe69/20260705_144319_ironclad_a4.jsonl` remains the latest Slime Boss split/minion-pressure evidence. `ai_runs_strategy_probe68/20260705_142530_ironclad_a4.jsonl` remains the latest Act 2 low-HP potion evidence.
...
- Latest validation: `python -m unittest discover -s tests` ran 172 tests OK after the non-attack damage regression; `python -m compileall slay_ai tests` OK. Probe70 reached F16 Hexaghost with 202 successful actions, 0 recovered action errors, 0 failed actions, and 41 one-turn-search decisions. It showed useful defensive search lines in Sentries/Fungi/Hexaghost, but the run still died because later replans drifted away from the full defensive sequence.
- Next live validation: continue Stage 4. Active bottlenecks are search sequence commitment across repeated state reads, Slime Boss split/minion handling, high-pressure potion/card-sequence search, Act 2 low-HP route/resource planning, and multi-target Sentries/Lagavulin/Hexaghost damage control.
