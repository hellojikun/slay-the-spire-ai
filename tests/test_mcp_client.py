...
class MCPClientTests(unittest.TestCase):
    ...
    def test_initialize_is_idempotent_for_reused_session(self):
        ...
        with patch("slay_ai.mcp_client.request.urlopen", return_value=response) as urlopen:
    ...
    def test_invalid_json_response_is_wrapped_as_mcp_error(self):
        client = MCPClient(timeout=0.1)
        with patch("slay_ai.mcp_client.request.urlopen", return_value=FakeResponse("<html>bad gateway</html>", None)):
            with self.assertRaises(MCPError) as raised:
                client.get_screen_state()
