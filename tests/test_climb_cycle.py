...
class ClimbCycleTests(unittest.TestCase):
    def test_skip_live_cycle_replays_logs_and_trains_gated_models(self):
        with TemporaryDirectory() as tmp:
            ...
            live_shadow = external_decision["live_shadow_disagreement"]
            self.assertEqual(live_shadow["status"], "evaluated")
            self.assertEqual(live_shadow["examples"], 1)
            self.assertTrue(Path(live_shadow["output_path"]).exists())
            self.assertFalse(live_shadow["model_authority"]["runtime_authority"])
            self.assertFalse(live_shadow["model_authority"]["direct_mcp_control"])
            self.assertEqual(live_shadow["promotion_readiness"]["status"], "shadow_only")
            self.assertFalse(live_shadow["promotion_readiness"]["can_promote_to_assist"])
            curriculum = external_decision["training_curriculum"]
            self.assertEqual(curriculum["status"], "active")
            self.assertGreater(curriculum["active_target_count"], 0)
            self.assertTrue(Path(curriculum["output_path"]).exists())
            ...
            self.assertEqual(external_decision["promotion_readiness"]["status"], "shadow_only")
            self.assertFalse(external_decision["promotion_readiness"]["can_promote_to_assist"])
            self.assertIn("external_prior_only_requires_live_shadow_validation", external_decision["promotion_readiness"]["blocking_reasons"])
            self.assertIn("take_skip", external_decision["prediction_audit"]["error_examples"])
            ...
            self.assertIn("External decision promotion readiness", review_text)
