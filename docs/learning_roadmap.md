...
- After probe84, the elite-chain fix was partially validated: the later F9 elite was avoided after the run was injured, but the run still reached Guardian under-resourced after a forced late Act 1 Sentries fight. This points to a broader route/resource model gap: current rest/shop/current-node buffers can hide the fact that the chosen path immediately commits into an elite before the boss with no potion and weak boss readiness. The next route work should be evidence-backed path-commitment planning, not a blind increase to a single elite penalty.
