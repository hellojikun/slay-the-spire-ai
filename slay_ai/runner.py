...
def _read_game_state(client: MCPClient, attempts: int = 8, delay: float = 0.25) -> dict[str, Any]:
    last_error: MCPError | None = None
    for attempt in range(attempts):
        try:
            return client.get_game_state(["player", "deck", "relics", "potions", "combat", "screen"])
        except MCPError as exc:
            last_error = exc
            if _is_transient_state_error(exc):
...
def _read_stable_game_state(client: MCPClient, attempts: int = 12, delay: float = 0.25) -> dict[str, Any]:
    state = _read_game_state(client, attempts=attempts, delay=delay)
    for _ in range(attempts):
        if _looks_stable(client, state):
            return state
        time.sleep(delay)
        state = _read_game_state(client, attempts=2, delay=delay)
    return state


def _looks_stable(client: MCPClient, state: dict[str, Any]) -> bool:
    if not state.get("in_game"):
        return True
    game = state.get("game_state", {})
    if game.get("screen_type") == "NONE" and game.get("room_phase") == "COMBAT":
        combat = game.get("combat_state", {})
        if _combat_snapshot_is_settling(combat):
            return False
        try:
...
def _combat_snapshot_is_settling(combat: dict[str, Any]) -> bool:
    ...
    if _combat_intents_are_settling(combat):
        return True
    ...
    if energy <= 0 or len(hand) >= 3:
        return False
    if _combat_has_current_turn_activity(combat):
        return False
    return True


def _combat_intents_are_settling(combat: dict[str, Any]) -> bool:
    if _combat_has_current_turn_activity(combat):
        return False
...
def _recover_state_read(client: MCPClient, interval: float) -> dict[str, Any]:
    time.sleep(max(interval, 0.5))
    return _read_stable_game_state(client, attempts=24, delay=0.2)
...
def _is_transient_state_error(exc: MCPError) -> bool:
    text = str(exc).lower()
    return ("internal error" in text and "null" in text) or "read_state_failed" in text


def _probe_mcp_health(client: MCPClient) -> dict[str, Any] | None:
