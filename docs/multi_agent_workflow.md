...
Updated 2026-07-05 after the probe72 clean-manifest and Weak-modeling round.
...
- Latest included probe: `ai_runs_strategy_probe72/20260705_153721_ironclad_a4.jsonl` ended as a clean F11 game-over after 183 action records, 0 recovered actions, and 0 failed actions. It live-triggered Weak-aware one-turn search decisions, but still died before the Act 1 boss with `Liquid Memories` unused. This makes the current bottleneck route/resource/potion/deck-quality evaluation rather than MCP execution. `ai_runs_strategy_probe71/20260705_151544_ironclad_a4.jsonl` remains the Hexaghost boss-pressure evidence; `ai_runs_strategy_probe69/20260705_144319_ironclad_a4.jsonl` remains the Slime Boss split/minion-pressure evidence.
...
- Latest learning run: `python -m slay_ai.training_manifest ...probe64 ...probe72 --output data\training_manifest_probe64_72.json --shadow-dir data\shadow_probe64_72` classified 9 clean logs, 0 diagnostic logs, and 0 infra-blocked logs, then wrote 128 route-risk, 484 potion-tempo, and 7 pre-boss deck-quality rows. A local probe64-72 card-model comparison loaded 67 card-pick examples and wrote 60 model deltas under ignored `models/`.
- Latest validation: `python -m unittest discover -s tests` ran 178 tests OK; `python -m compileall slay_ai tests` OK. Probe72 shows clean execution but earlier strategic failure, so the route should move to manifest-gated learning surfaces rather than more blind heuristic patching.
- Next live validation: continue Stage 4. Active bottlenecks are route-risk evaluation before forced damage, high-pressure potion/card-sequence search, Liquid Memories / potion tempo, pre-boss deck/resource quality, Hexaghost/boss high-pressure survival, Slime Boss split/minion handling, and early Act 2 low-HP route/resource planning.
