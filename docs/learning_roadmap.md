...
- Probe94 shows the A0 route is no longer blocked at the first boss: the run repeated an Act 1 clear and died later in Act 2 at F31. The next learning/control target should move from "reach Act 1 boss" toward Act 2 low-HP route risk, potion tempo under multi-enemy pressure, and terminal-state MCP quality; do not promote broad combat action learning from this single clean A0 sample.
