...
Updated 2026-07-06 after probe99, the live run-status monitor, A0 Act 1 boss gate integration, terminal recovery fix, CHEST skipped-relic validation, and run-log directory migration.
...
- Current short-term milestone: stabilize A0 Act 1 boss completion before resuming ascension climbing. Probe91 (`runs/ai_runs_strategy_probe91_a0/20260705_201624_ironclad_a0.jsonl`) reached F19 after beating Slime Boss but was manually stopped. Probe94 (`runs/ai_runs_strategy_probe94_a0/20260705_204121_ironclad_a0.jsonl`) repeated the Act 1 clear and reached Act 2 F31. Probe95 (`runs/ai_runs_strategy_probe95_a0/20260705_205543_ironclad_a0.jsonl`) again cleared Act 1 and reached Act 2 F21. Probe96 (`runs/ai_runs_strategy_probe96_a0/20260705_211219_ironclad_a0.jsonl`) died cleanly to Guardian at F16 after overusing emergency potions before a non-potion Mode Shift line. Probe97 (`runs/ai_runs_strategy_probe97_a0/20260705_232903_ironclad_a0.jsonl`) live-validated the potion-sequencing fix by clearing Guardian and reaching Act 2 F33, but hit max steps and had 3 recovered unavailable-action races, so it is diagnostic validation rather than pristine proof. Probe99 (`runs/ai_runs_strategy_probe99_a0/20260706_002051_ironclad_a0.jsonl`) reached Guardian and died at F16; it is `usable_with_recoveries` with 1 recovered action race, synthetic terminal recovery after MCP null, and `failure_attribution=deck_quality`. Probe100 (`runs/ai_runs_strategy_probe100_a0/20260706_002940_ironclad_a0.jsonl`) reached Hexaghost at 78/80 HP with two potions and died on T15; it has no synthetic terminal, but still has 1 recovered command-surface race and `failure_attribution=combat_planning`.
- Current cross-thread owner: user designated Codex session `019f32eb-a5d2-7a62-8cf5-e87c88a312a9` to continue this work. Do not run a second live MCP controller from this thread while that session owns the line; use this thread for documentation, offline manifest checks, and code/test work unless ownership changes.
...
- Campaign probe runs can now pass `--manifest-shadow-dir`, `--manifest-advice-dir`, all shadow model paths, and `--act1-boss-gate-output` through the run batch. Progress records `last_manifest`, clean/diagnostic/infra status, `last_shadow_advice`, manifest history, and `last_act1_boss_gate_*` fields, including `last_act1_boss_gate_progress`, `last_act1_boss_gate_remaining_progress`, `last_act1_boss_gate_next_probe_goal`, `last_act1_boss_gate_next_probe_acceptance_criteria`, `last_act1_boss_gate_latest_run_acceptance`, `last_act1_boss_gate_execution_recovery`, and `last_act1_boss_gate_data_quality`. Flow/data agents should use these flags for A0 probe batches so model advice, search-label review, execution-recovery context, data-quality context, and boss-gate acceptance stay paired with the run evidence without controlling the policy.
- Flow/data agents can refresh the route-risk, potion-tempo, pre-boss deck-quality, and combat-search shadow models with `python -m slay_ai.train_shadow_models <shadow dirs> --model-dir models --advice-output-dir <advice_dir> --summary-output <summary.json> --source-quality pristine`. The summary is part of the evidence: zero or tiny feature/card-prior counts mean the model is still diagnostic-only and should not be promoted. Shadow advice covers route, potion, deck-quality, and combat-search label-review rows; no policy code reads these model outputs.
- Flow/data agents should finish each probe batch with `python -m slay_ai.run_diagnosis <manifest> --advice-dir <shadow_advice_dir>` when advice exists. This produces the handoff line that tells the main thread whether the next lane is route risk, potion planning, deck quality, combat planning, or MCP execution. Each diagnosis now also carries `recommended_assignment` with owner, execution contract, compact evidence, and an assignment prompt; use `--assignment-output <txt>` when a single manifest diagnosis needs to be handed directly to the next worker. Diagnostic evidence-collection actions such as `exclude_and_collect_terminal_boss_evidence` and `exclude_and_collect_pristine_boss_clear` belong to `runner_agent` and require explicit live-MCP ownership, while execution repairs stay with `engineering_agent`.
- Flow/data agents should also run `python -m slay_ai.act1_boss_gate <manifest...> --output <gate_report.json>` after A0 probe batches. This is the current acceptance panel for Act 1 boss stability: reached count, cleared count, pristine cleared count, infra-blocked runs, unknown failure attribution, data quality, execution-recovery distributions, and failure-evidence distributions. Use `gate.remaining_progress` as the machine-readable handoff for what still blocks promotion; for the current probe94-103 batch it should point only at `pristine_act1_boss_cleared` with a deficit of 1. Use `gate.next_probe_goal.summary` as the human-readable next live-run target, `gate.next_probe_goal.acceptance_criteria` as the machine-readable contract for whether the next run can count toward the gate, and `gate.latest_run_acceptance.blockers` to explain why the latest matching run did or did not count. The CLI status line also includes `latest_accept=accepted` or a compact `latest_reject=...` suffix for monitors that only read stdout, top uncleared-boss and top non-pristine-boss attribution, compact failure-evidence/screen-stall/action-error/MCP-read/terminal-source suffixes, plus action/terminal recovery suffixes when gate-level execution recovery is present. `offline_batch` carries the same gate execution recovery, compact gate data quality, manifest failure evidence, and gate failure evidence into batch triage, runner/main-agent queue items, handoff JSON, and assignment prompts; feature coverage gaps, feature-zero rows, category-level `feature_issue_categories`, missing coverage, unknown static facts, or label exclusions also queue an AI-owned `review_gate_data_quality` task for offline shadow-data review, while stale manifest summary schemas queue an engineering-owned `refresh_stale_manifest_schema` task before strategy analysis relies on older artifacts. When `--knowledge-dir` is enabled, `offline_batch` also writes `static_knowledge_gaps_<name>.json` and attaches compact concrete card/potion/relic/monster gap samples to batch triage, handoff JSON, `next_handoff.txt`, and assignment prompts, so an AI worker can start from entity IDs instead of only aggregate unknown-static counters. `offline_batch` also writes `combat_label_audit_<name>.json` and attaches compact excluded combat-search label examples to `review_excluded_combat_search_labels`, so missed lethal and missed single-card-search labels can be reviewed before retraining. Use `python -m slay_ai.static_knowledge_gaps <logs...>` or `python -m slay_ai.combat_label_audit <shadow dirs...>` separately when you need one focused report without rebuilding the full offline batch. For the current pristine-clear lane, that contract requires `act1_boss.prefix_pristine_clear=true` and empty `act1_boss.prefix_blockers`, with recovered action races, failed actions, and synthetic/null terminal evidence disallowed before the Act 1 boss clear.
- Flow/data agents can read live or completed JSONL logs without touching MCP by running `python -m slay_ai.run_status <log-or-run-dir>`. This skips only a malformed trailing live-write line, prints per-run floor/screen/combat status, validation grade, manifest category/reason, recovered/failed action counts, compact action-recovery kind, failure attribution, validation flags, compact failure evidence for MCP read/null, synthetic terminal, action/preflight, reward-screen stalls with step range/count/last action, label exclusions such as missed direct kills, and compact Act 1 boss evidence, then prints one `run_status_summary` line with `classes=...`, `class_reasons=...`, and screen-stall type/reason rollups for the resolved batch. When `--output` is used, the JSON summary also rolls up state counts, clean/diagnostic/infra counts, classification reasons, validation grades and flags, terminal-recovery attempted/succeeded/failed counts for synthetic terminal states, action-recovery status/kind counts such as `stale_target_index`, `stale_potion_target_index`, or `stale_potion_slot`, combat-search label-quality exclusions, screen-stall screen/reason counts, Act 1 boss prefix/clear blockers, failure attributions, and failure-evidence kinds across all resolved logs.
- The same `run_status` JSON summary now includes `recommended_next_action`, `recommended_owner`, `recommended_reason`, `recommended_live_mcp_required`, `recommended_source_count`, up to five `recommended_source_paths`, matching compact `recommended_source_runs` with category/reason, failure attribution, validation grade, recovery counts, compact `failure_evidence` snippets, Act 1 boss evidence, and status line, plus a `recommended_assignment` block with source runs, exact input/resolved-log traceability, warnings, an execution contract, compact static-knowledge gaps, and an `assignment_prompt` that can be handed directly to the recommended agent for quick triage. The assignment prompt repeats the run-status summary, allowed/forbidden operations, source status lines, compact static-knowledge gaps, and compact structured evidence so a worker can start from the prompt alone. The CLI prints the compact form as a final `run_status_next:` line with execution mode, source count, and first source filename; use `--assignment-output <txt>` to write just the recommended assignment prompt for direct handoff, and `--static-knowledge-gaps-output <json>` to persist the concrete missing card/potion/relic/monster report from the same read-only monitor pass. Use this for cheap polling while a run is active or just ended, then use `run_diagnosis`, `act1_boss_gate`, or `offline_batch_next:` for the fuller handoff once artifacts are refreshed.
- `offline_batch` batch triage, handoff indexes, and assignment prompts include manifest-level `failure_evidence` rollups by evidence type, reward/chest screen-stall screen/reason, action-error kind/status, MCP read event/status, and synthetic-terminal source. The offline batch summary also mirrors gate action, progress, remaining progress, latest-run acceptance, next probe goal, recommended next action, and an `offline_batch_next_line` at top level for simple monitors; the CLI prints that same `offline_batch_next:` line with primary/coordinator tasks, live-MCP ownership flags, and queue counts. Compact `agent_queue_actions` entries include gate metric/deficit, acceptance scope, counts-toward metric, latest-run accepted state, and blockers for gate-owned tasks. Assignment prompts also include compact per-task `failure_evidence` for the first queued task when present, so engineering workers can see screen stalls, MCP read failures, action/preflight errors, and synthetic terminal recovery without opening the handoff JSON first. `next_handoff.txt` also mirrors the Act 1 boss gate status line, remaining progress, latest-run acceptance, next probe goal, gate failure evidence, and the primary worker's failure evidence in its top summary when available.
- CHEST issue: probe91 showed F9 and F17 CHEST states with `chest_open=false`, empty rewards, and only Burning Blood in the relic list. `slay_ai.policy_chest` now probes such unverified chests before proceeding, and runner snapshots log CHEST `chest_open` plus visible rewards. Probe94 live-validated this fix: F9, F17, and F26 chests were opened before proceeding, and F9/F26 logged `Collect RELIC`.
...
- Probe88 update: `ai_runs_strategy_probe88/20260705_194207_ironclad_a4.jsonl` is diagnostic-excluded. It stopped without a terminal outcome at F8 map state, so this is not a strategy death and writes zero shadow examples.
- Latest excluded diagnostic probes: `ai_runs_strategy_probe38/20260705_072307_ironclad_a4.jsonl` stopped at max steps due to a `SHOP_ROOM` / `SHOP_SCREEN` loop; the first probe39 attempt was aborted after proving `leave` is not a valid `execute_actions` action; probe43 and its continuation were split empty-hand diagnostics; probe45 and its two continuations were split Fiend Fire empty-hand diagnostics; probe46 (`ai_runs_strategy_probe46/20260705_085517_ironclad_a4.jsonl`) was stopped after exposing a `GRID` `confirm` / MCP `proceed` preflight loop; probe50 (`ai_runs_strategy_probe50/20260705_093533_ironclad_a4.jsonl`) and `ai_runs_strategy_probe50_continue_handselect_rewrite/20260705_093831_ironclad_a0.jsonl` are HAND_SELECT action-rewrite diagnostics; probe51 (`ai_runs_strategy_probe51/20260705_093933_ironclad_a4.jsonl`) ended as MCP unreachable/read_failed; probe52 (`ai_runs_strategy_probe52/20260705_095355_ironclad_a4.jsonl`) was manually stopped after exposing a Neow event GRID duplicate-card selection loop; probe60 (`ai_runs_strategy_probe60/20260705_124621_ironclad_a4.jsonl`) was manually stopped after exposing a multi-card `HAND_SELECT` rewrite gap, `ai_runs_strategy_probe60_continue_handselect_multichoose/20260705_125224_ironclad_a0.jsonl` is a partial continuation validation, `ai_runs_strategy_probe60_continue_act2/20260705_125715_ironclad_a0.jsonl` is a split continuation used only to diagnose Act 2 Byrds self-damage overuse, and probe88 is excluded as a no-terminal diagnostic run. Do not use these diagnostic logs in default training.
...
- Latest potion sequencing fix: after probe96, emergency potion policy preserves non-healing defensive/tempo potions when one-turn search already finds a survivable hand-only line. This specifically prevents spending Skill Potion or Fire Potion on Guardian turns where `Carnage`/`Pommel Strike` style damage can already trigger Mode Shift.
- Latest combat-search calibration: Slime Boss and large slime split thresholds now clear the modeled current attack when unblocked damage crosses half HP, and search target choice prioritizes damage that removes current attack intent. This gives local search the same intent-stop concept already used by single-card targeting, so a lethal Slime Boss turn can prefer a split trigger over insufficient block or a wrong target.
- Latest control-card search calibration: `Disarm` is now a supported one-turn search card. It reduces the modeled current attack by strength-down per hit, respects Artifact, and can combine with block in a searched survival line for multi-hit boss pressure such as Hexaghost.
- Latest damage/control setup search calibration: `Bash`, `Uppercut`, `Thunderclap`, and `Shockwave` now apply modeled Vulnerable after their own damage/control effects, later attacks use 1.5x projected damage, and Artifact can block the debuff. `Shockwave` also applies all-enemy strength down after Weak/Vulnerable, so high-pressure Hexaghost/elite search lines use its full control value. This improves Ironclad boss-output search labels such as `Bash -> Strike`, `Thunderclap -> Strike`, and `Shockwave -> Strike` while keeping learned models shadow-only.
- Latest upgrade-fact search calibration: combat search strips `+` suffixes from card ids/names for effect lookup and uses card-specific upgraded debuff deltas, so MCP snapshots like `Disarm+`, `Shockwave+`, or `Thunderclap+` do not undercount or overcount their modeled one-turn effects.
- Latest retaliation search calibration: combat search values `Flame Barrier` retaliation against attacking monsters and exports `retaliation_damage` through search metadata and combat-search labels. This helps Hexaghost/Guardian multi-hit boss lines and shadow value models without treating retaliation as pre-attack incoming removal.
- Latest missed-search diagnosis calibration: boss single-card diagnostics now accept avoided-lethal, large loss-reduction, and retaliation lines even when `attacks_removed=0`. Missed defensive cards such as `Flame Barrier` can be surfaced for AI review and excluded from imitation labels instead of disappearing as a clean combat-planning failure.
- Latest search-log calibration: `One-turn search` reasons now include damage dealt, attacks removed, and whether lethal was avoided, so flow/data agents can audit why a searched line was accepted without reconstructing the combat state.
- Latest structured-decision logging: search decisions now write `decision.metadata.search` into runner JSONL logs with sequence card ids, score, loss projection, kills, attacks removed, and avoided-lethal. Flow/data agents should prefer these structured fields over parsing the `reason` string.
- Latest search-label data export: `training_manifest` now writes `combat_search_labels.jsonl` from `decision.metadata.search` for clean trainable runs. These rows carry the searched sequence label plus compact HP, energy, hand, enemy, and loss-projection context for future imitation/value-model training.
- Latest combat-search shadow model: `slay_ai.train_combat_search_model` trains `models/combat_search_model.json` from `combat_search_labels` rows, learning rough first-card priors and context-card scores for offline label review. `shadow_advice` now writes `combat_search_advice.jsonl` with model-vs-search agreement signals, and `run_diagnosis` surfaces `review_search_label` as a watch signal; no policy code reads it.
- Latest probe86 policy addition: single-card combat scoring gives `Second Wind` a narrow Burn/Burn+ cleanup bonus under Hexaghost or low-HP burn pressure. This specifically covers probe86's low-HP no-incoming Hexaghost turn where leaving `Burn+` in hand made the following attack lethal; plain Dazed/status hands are still left to normal scoring.
- Latest probe87 policy addition: pressure attack fallback now skips self-damage attacks such as `Hemokinesis` when projected HP after self-damage and incoming is critically low, unless the attack safely ends the fight. This prevents fallback from bypassing the existing single-card self-damage risk penalty.
- Latest route policy fix: route decisions now attach a best-effort full-map observation only on `MAP` screens, with a 2.5s timeout and per-episode failure disable after repeated MCP errors. When the map payload can be parsed, `policy_route` evaluates a bounded 6-layer lookahead and applies path-commitment risk for low-HP forced elite/combat routes while logging base score, lookahead adjustment, final score, node count, and edge count. If map observation fails, route policy falls back to the old immediate `next_nodes` scoring. After probe61, Act 2 low-HP route scoring penalizes immediate hallway fights when a question mark is available, so equal-risk lookahead no longer lets the monster base score dominate. After probe63, Act 1 routes that force an elite within 3 nodes without a rest/shop buffer are penalized, while nearby shop/rest buffers receive an explicit survival bonus. After probe66, the no-buffer forced elite penalty was strengthened so a full-HP path does not lock into an early elite when a slightly lower-score question path has later recovery. After probe73, high-HP Act 1 paths that still force an elite within 3 nodes with no rest/shop buffer receive a stronger penalty, so a visible shop/rest-buffer route can win before the path is locked. After probe83, injured immediate-elite choices that force another elite within 3 nodes before close rest/shop recovery receive an `act1_elite_chain_penalty`. After probe84, injured rest choices receive an `act1_rest_forced_elite_penalty` when the rest node still commits to a close forced elite before boss readiness is adequate.
...
- Current policy split status: combat reward, card reward, potion timing, rest/grid, event/hand-selection, shop/relic, chest, route, combat orchestration, and character profile facts now have separate policy modules. `slay_ai.policy_character` owns starter-card and Ironclad-specific card fact sets used by the current heuristic, while `slay_ai.policy_combat.CombatPolicy` owns the combat action ordering layer. The detailed single-card scoring and search acceptance helpers still live in `policy.py`.
...
- Latest probe95 manifest: `data\training_manifest_probe95_a0.json` classified 1 clean log, 0 diagnostic logs, and 0 infra-blocked logs, with 19 route-risk rows, 163 potion-tempo rows, and 1 pre-boss deck-quality row. The run had 412 ok actions, 3 recoverable action races, 0 failed actions, cleared Act 1 at low HP, and died at Act 2 F21 to triple Cultists. The terminal event records `terminal_recovery_attempted=true` and `terminal_recovery_succeeded=true`; a follow-up watchdog confirmed MCP healthy at `MAIN_MENU`.
- Latest probe96 manifest: `data\training_manifest_probe96_a0.json` classified 1 clean log, 0 diagnostic logs, and 0 infra-blocked logs, with 15 route-risk rows, 84 potion-tempo rows, and 1 pre-boss deck-quality row. The failure attribution is `deck_quality`, but the concrete local fix is potion sequencing: emergency potion policy now asks one-turn search whether the current hand can already handle the danger before spending non-healing defensive/tempo potions.
- Latest probe97 manifest: `data\training_manifest_probe97_a0.json` classified 0 clean logs, 1 diagnostic log, and 0 infra-blocked logs because the campaign stopped at `max_steps=550`. The new validation summary records `act1_boss_reached=1`, `act1_boss_cleared=1`, and `pristine_act1_boss_cleared=0`; item evidence shows Guardian entry at 80/80 HP with 3 potions, only one boss-fight potion use at step 233, and Act 1 clear before the diagnostic step cap.
- Latest probe98 manifest: `data\training_manifest_probe98_a0.json` classified 0 clean logs, 1 diagnostic log, and 0 infra-blocked logs. This thread stopped its live MCP controller after the user designated session `019f32eb-a5d2-7a62-8cf5-e87c88a312a9` as the owner, so the run has `no_terminal_outcome` and must not train models. Validation evidence still records Act 1 boss reach: Slime Boss at F16, entry HP 78/80 with 2 potions, last observed boss turn 8 at HP 35, one boss potion-use step, and 1 recovered unavailable-action race.
- Latest manifest validation fix: `training_manifest` now annotates each run with `validation_grade`, `validation_flags`, and `validation_evidence.act1_boss`. This separates training eligibility from boss-stability evidence: max-step or recovered-race logs can preserve Act 1 clear evidence while staying out of pristine validation counts. The Act 1 boss detector now avoids treating Act 2 `SphericGuardian` as The Guardian.
- Latest runner start fix: runner still recovers lingering terminal `GAME_OVER` screens that expose only `proceed`, and now also detects existing-save blocks before starting a fresh run. If `get_available_commands` incorrectly reports `start_game`, runner parses MCP `Possible commands` from the failed `start_game` call; with `existing_save=fail` it raises a clear existing-save error before retrying start, and with `existing_save=abandon` it abandons the lingering save before retrying start.
- Latest runner transition fix: probe98 exposed a benign F11 map-transition race where the state snapshot had already reached `MAP` and selected a route, but MCP still exposed only `proceed` from the previous reward transition. Preflight now rewrites only this narrow `MAP choose` + `proceed-only` shape into `proceed`, then the next frame re-reads the map and chooses the route normally. This should reduce recovered unavailable-action races without letting runner choose a route on stale command state.
- Latest diagnosis fix: `run_diagnosis` now surfaces `validation_grade` plus compact `validation_evidence.act1_boss` in both JSON output and status lines. Probe98 now diagnoses as `act1_boss=reached:SlimeBoss[T8,hp=35,potions=1]` and recommends collecting terminal boss evidence instead of treating the run as a generic incomplete diagnostic.
- Latest campaign progress fix: campaign progress records now copy `validation_grade`, `validation_flags`, and compact Act 1 boss evidence from each per-run manifest into `data\campaign_*.json`. Flow agents can see `last_act1_boss_reached`, `last_act1_boss_cleared`, boss enemy ids, entry HP/potions, last observed boss turn/HP, and boss potion-use steps without opening the manifest file.
- Latest shadow-data quality fix: all `training_manifest` shadow rows now include source-quality fields copied from the classified run: `source_category`, `source_reason`, `source_validation_grade`, `source_validation_flags`, `source_recovered_actions`, `source_failed_actions`, `source_has_recovered_action_race`, and `source_action_recovery_kinds`. Shadow/deep models should prefer `source_validation_grade=pristine` for strict training and use `usable_with_recoveries` rows only for diagnostics, downweighted training, or explicit ablations.
- Latest shadow-trainer quality gate: `train_route_risk_model`, `train_potion_tempo_model`, `train_deck_quality_model`, `train_combat_search_model`, and aggregate `train_shadow_models` now default to `--source-quality pristine`. Use `--source-quality usable` only for explicit recovered-race ablations/diagnostics, and `--source-quality legacy` only when intentionally reusing old ungraded shadow files.
- Latest Act 1 boss gate report: `data\act1_boss_gate_probe94_103_a0_full.json` summarizes probes94-103 as `runs=10`, `reached=10/3`, `cleared=5/2`, `pristine_cleared=1/2`, `infra=0`, and `unknown_attr=0`. The current next action remains `collect_pristine_act1_boss_clears`, not a broad strategy rewrite. The gate now counts Act 1 boss prefix-pristine clears, so probe102's clean Guardian clear counts even though later Act 2 execution races made the full run `usable_with_recoveries`. The top non-pristine clear blocker remains recovered actions before or during Act 1 clear evidence.
- Latest probe99 gate report: `data\act1_boss_gate_probe99_a0.json` is a single-run failure report with `reached=1`, `cleared=0`, `pristine_cleared=0`, `recovered_action_runs=1`, validation grade `usable_with_recoveries`, and `failure_attribution=deck_quality`. The live status line is `run_status terminal: floor=16 act=1 screen=GAME_OVER phase=COMPLETE step=261 hp=0/80 actions=260 recovered=1 failed=0 validation=usable_with_recoveries failure=deck_quality flags=recovered_action_race,synthetic_terminal,synthetic_after_mcp_null,+1 act1_boss=reached:TheGuardian[player_hp=0]`.
- Latest probe100 gate report: `data\act1_boss_gate_probe100_a0.json` is a single-run Hexaghost failure report with `reached=1`, `cleared=0`, `pristine_cleared=0`, `recovered_action_runs=1`, validation grade `usable_with_recoveries`, and `failure_attribution=combat_planning`. The combined `data\act1_boss_gate_probe94_100_a0.json` now has `runs=7`, `reached=7/3`, `cleared=3/2`, `pristine_cleared=0/2`, `infra=0`, and `unknown_attr=0`; it still needs pristine boss clears. The probe100 recovered race is a new map-to-combat first-frame command-surface issue: JSON state briefly reported `NONE/COMBAT` at F14 while MCP still exposed `choose/confirm` for Gambling Chip `HAND_SELECT`.
- Latest runner freshness fixes: probe99's stale shop/reward `proceed`, probe100's map-to-combat `HAND_SELECT` command surface, probe101/probe102 MAP `choose`/`proceed` double-stale surfaces, probe101's `DistilledChaos` post-potion combat-surface lag, probe101's GRID `choose` command-surface lag, and probe102's combat-to-reward post-transition `end_turn`/`play_card` errors now each have a narrow wait/re-read branch. Probe102 cleanly reached and cleared Guardian with `recovered=0` through the Act 1 boss; probe103 then cleanly reached Guardian with `recovered=0` but died at F16 Guardian with `failure=combat_planning` and synthetic terminal flags.
- Latest validation: `python -m unittest discover -s tests` passed 339 tests, `python -m compileall slay_ai tests` passed, and `git diff --check` passed apart from CRLF warnings.
- Simulator supplement: consider a separate offline lane using `conquer-the-spire` (https://github.com/utilForever/conquer-the-spire), a C++17 Slay the Spire simulator with C++/Python APIs and RL-oriented support. Use it for pretraining and heuristic stress tests only; MCP live runs remain the source of validation grades and final performance claims.
- Next live validation: the designated session `019f32eb-a5d2-7a62-8cf5-e87c88a312a9` owns the live line. Use probe104/next fresh A0 to collect one more Act 1 boss prefix-pristine clear and to validate that the combat-to-reward and conservative MAP waits stay clean beyond Act 1. Strategy work should focus on Guardian combat planning and terminal/null recovery rather than broad A20 expansion.
...
| --- | --- | --- | --- |
| External Codex session | Current live A0 validation owner | `019f32eb-a5d2-7a62-8cf5-e87c88a312a9` | User-designated session for continuing the A0 Act 1 boss / execution-layer work. Other sessions should not drive the live MCP game concurrently. |
| Archimedes | Flow / run-data agent | `019f30f8-ed4d-7bd3-9519-b3bcfda8116d` | Run frontier probes, accumulate climb logs, classify whether failures are clean/diagnostic/infra, and report concrete bottlenecks. Does not edit code. Latest completed assignment was superseded by main-thread probe88; next assignment is probe89 after this manifest-quality fix is pushed. |
| Harvey | AI upgrade / algorithm agent | `019f2e94-552b-7301-9cc5-21eed1fc2949` | Static features, readiness gates, shadow models, training data design, and model promotion gates. Does not edit code unless explicitly reassigned. Current status: not available in current tool state; fresh AI-worker spawn on 2026-07-05 failed with `agent thread limit reached`. Recreate or resume when tool capacity allows. |
| Main Codex agent | Program optimization / integration / temporary AI algorithm owner | current thread | Owns code edits, tests, commits, pushes, and conflict control while subagent capacity is capped at one active child agent. Temporarily covers program optimization and AI algorithm work until separate workers can be spawned. |
...
2026-07-05 current three-role parallelization attempt:

- User direction: one agent should run process and accumulate climb information, one should handle program/script optimization, and one should handle AI/model/algorithm upgrades.
- Active subagent state from the environment: only Archimedes (`019f30f8-ed4d-7bd3-9519-b3bcfda8116d`) is currently visible.
- Flow-agent command:
...
multi_agent_v1.send_input(Archimedes, "Continue as the flow/run-data agent. After the main thread pushes the probe87 self-damage fallback fix, pull/update if needed, then run probe88 with ai_runs_strategy_probe88 and data\\campaign_strategy_probe88.json. Generate data\\training_manifest_probe88.json and data\\shadow_probe88. Do not edit code or commit; report clean/diagnostic/infra status and the next concrete bottleneck.")
multi_agent_v1.send_input(Archimedes, "Continue as the flow/run-data agent. After the main thread pushes the probe88 manifest-quality fix, run probe89 with ai_runs_strategy_probe89 and data\\campaign_strategy_probe89.json. Generate data\\training_manifest_probe89.json and data\\shadow_probe89. Do not edit code or commit; report clean/diagnostic/infra status and the next concrete bottleneck.")
```

- Program-agent spawn attempt:

```text
multi_agent_v1.spawn_agent(worker, "Program optimization agent: inspect runner/campaign/MCP/logging stability, avoid policy_route.py and combat_search.py, report one low-conflict engineering improvement before editing.")
```

- AI-agent spawn attempt:

```text
multi_agent_v1.spawn_agent(worker, "AI upgrade/algorithm agent: inspect policy_route.py, combat_search.py, tests, and probe82-84 evidence; propose one minimal measurable algorithm improvement before editing.")
```
...
- Probe87 result: clean F16 Hexaghost loss with no execution failures. Main thread regenerated the full manifest/shadow after the run completed and added a targeted fallback self-damage fix for the `Hemokinesis` death chain.
- Probe88 result: main thread ran probe88 because no background artifact appeared. The latest local artifact stopped without a terminal outcome at F8 map state; main thread regenerated the manifest/shadow outputs and added a guard so any future `synthetic_after_main_menu` terminal source is diagnostic-only.
- 2026-07-05 current handoff: main thread attempted to spawn a separate program-optimization worker, but the multi-agent tool returned `agent thread limit reached`. Archimedes received the probe89 flow assignment; main thread temporarily owns program and AI-algorithm work until agent capacity is available.
...
2026-07-05 probe83 result:

- Command family: current-frontier Ironclad A4 campaign probe, output in `ai_runs_strategy_probe83`.
- Manifest: `data\training_manifest_probe83.json`.
- Shadow data: `data\shadow_probe83`.
- Category: 1 clean trainable completed failure.
- Failure: F16 Hexaghost, HP 6, no potions, boss still at 29/250. Nob Rage did not recur, and Burn/Burn+ loss accounting got live coverage without reproducing the probe81 undercount.
- Flow-agent note: the clearest upstream issue was F7 choosing an immediate elite path that forced another elite within three nodes before close recovery. At F9 the elite was already forced, so the fix belongs at the F7 route-commitment decision.
- Program fix: add an Act 1 elite-chain low-buffer penalty for injured immediate-elite choices when another elite is forced before nearby rest/shop recovery.

Probe84 should run only after the elite-chain route fix is committed and pushed:

```powershell
python -m slay_ai.campaign --characters IRONCLAD --ascension 20 --attempts-per-target 1 --max-steps 440 --interval 0.08 --startup-timeout 20 --cooldown 0.5 --existing-save fail --progress-file data\campaign_strategy_probe84.json --log-dir ai_runs_strategy_probe84 --use-all-hardware
...
```powershell
python -m slay_ai.campaign --characters IRONCLAD --ascension 20 --attempts-per-target 1 --max-steps 260 --interval 0.08 --startup-timeout 20 --cooldown 0.5 --existing-save fail --progress-file data\campaign_strategy_probe.json --log-dir runs\ai_runs_strategy_probe --use-all-hardware
```
...
```powershell
python -m slay_ai.runner --max-steps 180 --interval 0.08 --startup-timeout 8 --log-dir runs\ai_runs_continue_probe
```
