...
Updated 2026-07-05 after the probe75 multi-agent flow/readiness validation round.
...
- Latest included probe: `ai_runs_strategy_probe75/20260705_162852_ironclad_a4.jsonl` ended as a clean F12 game-over after 129 ok actions, 1 recovered preflight mismatch, and 0 failed actions. It reached a second elite/multi-enemy fight and died from long pressure with no potion-tempo rows, so the current bottleneck moved from F10 hallway death to Act 1 readiness/elite path commitment. `ai_runs_strategy_probe74/20260705_161424_ironclad_a4.jsonl` remains the clean F10 Jaw Worm + Acid Slime regression case with unused `CultistPotion`. `ai_runs_strategy_probe73/20260705_160652_ironclad_a4.jsonl` remains the clean F6 Sentries route-readiness regression case: F3 chose a no-buffer forced-elite path over a shop, then died with no AOE/Weak and low HP. `ai_runs_parallel_flow/20260705_155559_ironclad_a4.jsonl` is the flow-agent confirmation of the same route/resource bottleneck: F7 had 37/88 HP and the route was already locked into `T -> E`.
...
- Latest learning run: `python -m slay_ai.training_manifest ...probe64 ...probe72 --output data\training_manifest_probe64_72.json --shadow-dir data\shadow_probe64_72 --knowledge-dir data\static_knowledge` classified 9 clean logs, 0 diagnostic logs, and 0 infra-blocked logs, then wrote static-knowledge and readiness-enriched route-risk, potion-tempo, and pre-boss deck-quality rows. The flow-agent manifest `data\training_manifest_parallel_flow.json` classified 1 clean log, probe73 manifest `data\training_manifest_probe73.json` classified 1 clean log, probe74 manifest `data\training_manifest_probe74.json` classified 1 clean log with 33 potion-tempo rows, and probe75 manifest `data\training_manifest_probe75.json` classified 1 clean log with 13 route-risk rows.
- Latest validation: targeted CultistPotion/static-knowledge/readiness tests passed, then probe75 reached F12 with clean execution. Run full `python -m unittest discover -s tests`, `python -m compileall slay_ai tests`, and `git diff --check` before commit/push.
- Next live validation: run probe76 after deciding whether readiness v3 should remain shadow-only or apply a route/shop/card-reward veto. Active bottlenecks are Act 1 elite path commitment while `aoe_missing`, `weak_missing`, `elite_potion_missing`, and `premium_block_missing` are flagged, high-pressure potion/card-sequence search, pre-boss deck/resource quality, Hexaghost/boss survival, Slime Boss split/minion handling, and early Act 2 low-HP route/resource planning.
