...
- After probe79, the first-AOE change did produce AOE (`Thunderclap`), but the run still died to Guardian with no potion and a high-pressure draw-order miss. At 11 HP and 36 incoming, `Battle Trance` was played only after the policy spent energy on block, so newly drawn zero-cost attacks could not combine with remaining energy to force Mode Shift. The next Stage 4 combat-search target is high-pressure draw/order planning before spending the last energy, not full combat imitation learning.
