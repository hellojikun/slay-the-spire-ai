...
Updated 2026-07-05 after the A0 Act 1 boss repeat probes, terminal recovery fix, CHEST skipped-relic validation, and run-log directory migration.
...
- Next live validation: after the target-index freshness fix, run another A0 probe under `runs/ai_runs_strategy_probe96_a0` to verify repeated Act 1 clear, clean terminal handling, and fewer recoverable action races. Active bottlenecks are target index staleness after monster death/split, action/state freshness around reward/choice transitions, Act 2 multi-enemy scaling fights such as triple Cultists, potion sequencing, and MCP `GAME_OVER` null-state serialization until MCPTheSpire itself is patched.
